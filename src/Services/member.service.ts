...
getMemberById(idcourant:string):Observable<User>{
  ...
  return new Observable(observer=>observer.next( this.tab.filter(item=>item.id==idcourant)[0]??null //bech ilwej el mmemeber selon id ou yebthou le subsriber pour l'editer( subscriber(x))
